...
from py_files.ml_dataset_prep import get_data_view, get_data_query,\
     get_data_validate_view, get_data_query_validate, get_data_query_predict, get_list_of_ticker_with_count, \
...
dates_to_train = [# 20251212, 20251217,20251230
                       #20260102, 20260106, 
                     #20260112, 20260115,
                      # 20260120, 20260218,# 20260220,
                      20260303,20260313,#20260323,
                    20260325]
_interval = 10
...
def _prepare_ds(**kwargs):
    ...
    df3 = get_zscore_difference(df3, interval = _interval)
    ...
    tickers_to_predict = X_pred[["ticker"]]
    X_pred.drop(columns = ["ticker", "difference"], inplace = True, errors = 'ignore')
    ...
    for cut_off, model in models.items():
        ...
        dictionary_or_results_predict = { "dt_pred": dt.predict(X_pred).tolist() \
            , "gbc_pred": gbc.predict(X_pred).tolist(), "svm_pred": svm.predict(X_pred).tolist(),\
               "ct_pred": ct.predict(X_pred).tolist() }
        temp_pred= pd.DataFrame(dictionary_or_results_predict )

        temp_pred.reset_index(inplace=True)
        temp_pred.drop(columns=["index"], inplace = True)
        tickers_to_predict.reset_index(inplace=True)
        tickers_to_predict.drop(columns=["index"], inplace = True)

        results_predict  = pd.concat([tickers_to_predict, temp_pred], axis = 1)
        results_predict["suma"] = results_predict["dt_pred"]+results_predict["gbc_pred"]+results_predict["svm_pred"]+results_predict["ct_pred"]

        results_predict.to_csv(f"/opt/airflow/dags/output_files/data_to_predict_{dt_string_with_hour}__{cut_off}.csv", index = False)
