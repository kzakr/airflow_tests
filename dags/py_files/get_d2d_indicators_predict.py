...
def get_statistical_metrics_avg(ticker_conditions= [], to_date:int = 20260401) -> str:
    ...
    interval_end = add_working_days(date_as_int = to_date,num_days = -1)
    ...
    sql_query_avg += f"where full_date> {interval_start} and full_date < {interval_end}\n"
...
def get_statistical_metrics_std(statistical_metrics_avg: str = get_statistical_metrics_avg(), ticker_conditions =[], to_date:int = 20260401) -> str:
    ...
    sql_query_std += f"\t on fr.ticker = avg.ticker where fr.full_date > {interval_start} and fr.full_date < {interval_end})\n"
    sql_query_std += f"where full_date> {interval_start} and full_date < {interval_end}\n"
    if ticker_conditions:
