...
def get_raw_data_change_yahoo(days_back: int) -> str:
    interval_start = add_working_days(num_days = -days_back, format = "%Y-%m-%d")

    sql_query = "(select ticker, _date, close AS close, volume as volume from yahoo_result \n"
    sql_query += "where  \n"
    sql_query += f" _date > DATE(\'{interval_start}\')  \n"
...
def get_closed_price(source:str, partition_list:str = ["ticker", "date"]):
...
def get_lagged_values(source:str, lags:int, lagged_column:str):
    ...
    for column in lagged_column:
        for i in range(1, lags):
            sql_query += f"\n\t, lag({column}, {i}) OVER (PARTITION BY ticker ORDER BY _date) AS {column}_{day_dict[i]}_day_back"
        ...
        for column in  lagged_column:
            for lag in range(min_iterated_list,max_iterated_list):
                ...
                if lag +1 < max_iterated_list:
                    ...
                    sql_query += f"\n\t{column}_{day_dict[lag]}_day_back {sign} {multiplier}*{column}_{day_dict[lag+1]}_day_back ) then add_working_days_date(_date, {max_iterated_list})\n"
