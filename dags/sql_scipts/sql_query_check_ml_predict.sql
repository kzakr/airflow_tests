drop view if exists ml_temp_predict; 
create view ml_temp_predict as select * from (with a as ( 
...
where  
 full_date in (select distinct full_date from finviz_result order by full_date desc limit 55)  
...
where full_date> to_char(CURRENT_DATE - INTERVAL '40 day', 'YYYYMMDD')::integer
group by ticker 
...
and avg(volume)<>0 and avg(price)<>0 
	 ...
	 on fr.ticker = avg.ticker where fr.full_date > to_char(CURRENT_DATE - INTERVAL '40 day', 'YYYYMMDD')::integer)
where full_date> to_char(CURRENT_DATE - INTERVAL '40 day', 'YYYYMMDD')::integer
