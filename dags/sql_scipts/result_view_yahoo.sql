...
, ranked_data AS (
  SELECT *, row_number() OVER (PARTITION BY ticker ORDER BY current_date_ DESC, _date DESC) AS rank
  FROM (
    select-- tickers_increasing.ticker, tickers_increasing.current_date_ , tickers_increasing._date,
     tickers_decreasing.ticker, tickers_decreasing._date, tickers_decreasing.days_back, tickers_decreasing.current_date_

  from  tickers_decreasing
